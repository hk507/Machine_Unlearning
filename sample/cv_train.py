...
from utils import make_logdir, union, Timer, TableLogger, parse_args, extract_mask
...
def compute_loss_ce(model, batch, args):
    images, targets = batch
    pred = model(images)
    loss = ce_criterion(pred, targets.to(torch.int64))
    accuracy = accuracy_metric(pred, targets.to(torch.int64))
    return loss, accuracy
...
def train(model, opt, lr_scheduler, train_loader, test_loader,
    ...
    for epoch in range(math.ceil(args.num_epochs)):
        ...
        train_loss, train_acc, download, upload = run_batches(
            model, opt, lr_scheduler, train_loader, test_loader,
        ...
        # val
        test_loss, test_acc, _, _ = run_batches(
            model, None, None, test_loader, None, False, 1, args
        )
        ################################################################################
        test_time = timer()
        ...
        if args.do_checkpoint:
            if epoch == args.num_epochs - 1:  # 确保这是最后一个epoch
                if not os.path.exists(args.checkpoint_path):
                    os.makedirs(args.checkpoint_path)
                torch.save({'state_dict': model.state_dict()},
                           args.checkpoint_path + args.model + '.pt')
...
def run_batches(model, opt, lr_scheduler, loader, val_loader,
...
if __name__ == "__main__":
    ...
    # fixupresnet9
    args = parse_args(default_lr=0.1)
    ...
    if args.mode == 'sketch':
        # Log
        sys.stdout = Logger(
            f"logs/mode = {args.mode} -- dataset name = {args.dataset_name} -- k num = {args.k} -- worker num = {args.num_workers} -- client num = {args.num_clients} -- {args.num_rows} + {args.num_cols}.txt")
    elif args.mode == 'new_sampling' or args.do_dp:
        if args.mode == 'fedavg':
            sys.stdout = Logger(
                f"logs/mode = {args.mode} -- dataset name = {args.dataset_name} -- k num = {args.k} -- worker num = {args.num_workers} -- client num = {args.num_clients} -- {args.epsilon} + {args.noise_multiplier} + {args.dp_mode}.txt")
        else:
            sys.stdout = Logger(
                f"logs/mode = {args.mode} -- dataset name = {args.dataset_name} -- k num = {args.k} -- worker num = {args.num_workers} -- client num = {args.num_clients} -- {args.epsilon} + {args.noise_multiplier}.txt")
    else:
        sys.stdout = Logger(
            f"logs/mode = {args.mode} -- dataset name = {args.dataset_name} -- k num = {args.k} -- worker num = {args.num_workers} -- client num = {args.num_clients}.txt")
    ...
    # 创建模型配置字典
    model_config = {'block': BasicBlock, 'layers': [2, 2, 2, 2], 'num_classes': num_classes, 'zero_init_residual': False, 'groups': 1,
                    'width_per_group': 64, 'replace_stride_with_dilation': [False, False, False],
                    'norm_layer': nn.BatchNorm2d}

    # # 实例化模型
    # model_cls = getattr(models, args.model)  # 获取模型类
    # model = model_cls(**model_config)  # 使用配置字典实例化模型
    ...
    if args.model[:5] == "Fixup":
        ...
        model.load_state_dict(torch.load(
            args.finetune_path + args.model + '.pt'))
    ...
    def lambda_step(step): return lr_schedule(step / spe)
