...
class FedModel:
    ...
    def _call_train(self, batch):
        ...
        elif self.args.mode in ["sampling","new_sampling"]:
...
class FedOptimizer(torch.optim.Optimizer):
    def __init__(self, optimizer, args):
        ...
        elif args.mode in ["true_topk", "local_topk", "sampling", "fedavg","new_sampling",
                           "uncompressed", "nips"]:
...
def get_server_update(gradient, Vvelocity, Verror, args, lr):
    helper = {"sketch": _server_helper_sketched,
              ...
              "new_sampling": _server_helper_new_sampling,  # New sampling-wjh
...
def _server_helper_fedavg(avg_update, Vvelocity, Verror, args, lr):
    ...
    if args.do_dp and args.dp_mode == "server":
        ps_update = clip_grad(args.l2_norm_clip, ps_update)
        # noise = torch.normal(mean=0, std=args.noise_multiplier,
        #                  size=ps_update.size()).to(args.device)
        # args.noise_sum += noise.abs()
        # ps_update += noise

        eps = args.epsilon / (args.num_epochs * 1)
    ...
    return ps_update * lr, Vvelocity, Verror
...
def _server_helper_sampling(sampling_grad, Vvelocity, Verror, args, lr):
    ...
    return update * lr, Vvelocity, Verror
def _server_helper_new_sampling(new_sampling_grad, Vvelocity, Verror, args, lr): # new sampling - wjh
    # this part for Outlier Elimination (in Minmax paper), so we do not need to make any change.
    assert args.error_type in ["local", "none"]

    est, sigma_w_e, sigma_w_r, sigma_C = torch.split(
        new_sampling_grad, [args.grad_size, args.grad_size, args.grad_size, 1])

    x = sigma_w_e ** 3 / (sigma_w_e ** 2 + sigma_C) / sigma_w_r
    x = torch.where(x.isinf(), torch.zeros_like(x), x)
    x = torch.where(x.isnan(), torch.zeros_like(x), x)

    new_sampling_grad = torch.where(
        x > args.outlier_thres, torch.zeros_like(est), est)

    rho = args.virtual_momentum
    torch.add(new_sampling_grad, Vvelocity, alpha=rho, out=Vvelocity)
